...
class AnalysisRequest(BaseModel):
    ...
    enable_scene_detection: Optional[bool] = True
    enable_nsfw_detection: Optional[bool] = True
    enable_violence_detection: Optional[bool] = True
    enable_black_scene_detection: Optional[bool] = True
    enable_frozen_scene_detection: Optional[bool] = True
    enable_audio_detection: Optional[bool] = True
    enable_ocr_detection: Optional[bool] = True
...
################OCR识别######################################################
    ...
    def analyze_video(
        ...
        enable_black_scene_detection: bool = True,
        enable_frozen_scene_detection: bool = True,
        ...
        for frame_count in range(total_frames):
            ...
            # 黑场检测
            if enable_black_scene_detection:
                ...
                if self._is_black_frame(frame):
                    ...
                    if (black_frame_count*sample_rate) >= self.BLACK_DURATION_THRESHOLD:
                        # 记录黑场异常
                    ...
                    black_frames_to_save = []
            # 静帧检测
            if enable_frozen_scene_detection:
                if self._is_frozen_frame(prev_frame, frame):
                    ...
                    if (frozen_frame_count*sample_rate) >= self.FROZEN_THRESHOLD:
                        # 记录静帧异常
                        # print("监测到静止帧")
        ...
        with open(ocr_csv_path, 'w', newline='', encoding='utf-8') as f:
            ...
            for ocr_frame in results['ocr']:
                if ocr_frame['text']:
                    # 展平嵌套列表（方法1）
                    text_content = '|'.join(
                        text for sublist in ocr_frame['text'] for text in sublist
                    )
                    writer.writerow([
                        ocr_frame['frame_num'],
    ...
    def create_api_app(cls):
        ...
        async def analyze_video_api(
            ...
            # 新增检测开关参数
            enable_face_detection: bool = Form(True, description="Enable face detection"),
            enable_scene_detection: bool = Form(True, description="Enable scene detection"),
            enable_nsfw_detection: bool = Form(True, description="Enable NSFW detection"),
            enable_violence_detection: bool = Form(True, description="Enable violence detection"),
            enable_black_scene_detection: bool = Form(True, description="Enable black scene detection"),
            enable_frozen_scene_detection: bool = Form(True, description="Enable frozen scene detection"),
            enable_audio_detection: bool = Form(True, description="Enable audio detection"),
            enable_ocr_detection: bool = Form(True, description="Enable OCR detection")
        ):
            ...
            print("敏感词路径:",sensitive_words_list)
            try:
                ...
                results = analyzer.analyze_video(
                    ...
                    enable_black_scene_detection=enable_black_scene_detection,
                    enable_frozen_scene_detection=enable_frozen_scene_detection,
                    enable_audio_detection=enable_audio_detection,
                    enable_ocr_detection=enable_ocr_detection
                )
